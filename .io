if game.PlaceId == 228181322 then  --- Dinosaur Simulator
    loadstring(game:HttpGet(('https://pastebin.com/raw/sFLTiDdx')))()
          elseif game.PlaceId == 292439477 then --- phamtom Forecs
    loadstring(game:HttpGet(('https://pastebin.com/raw/XqJguyKe')))()
          elseif game.PlaceId == 6516141723 then --- Doors moblie
    loadstring(game:HttpGet(('https://pastebin.com/raw/fiW2wsQe')))()
              elseif game.PlaceId == 537413528 then --- Build A Boat For Treasure
    loadstring(game:HttpGet(('https://pastebin.com/raw/b3cxkFmp')))()
                  elseif game.PlaceId == 9872472334 then --- Evade
    loadstring(game:HttpGet(('https://pastebin.com/raw/QUwKLmX1')))()
                      elseif game.PlaceId == 2753915549 then --- Blox Fruits
    loadstring(game:HttpGet(('https://pastebin.com/raw/H41vgeDa')))()
end
